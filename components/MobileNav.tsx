...
const IconComponent = ({ link }: { link: socialLinksType }) => {
    return (
        <a href={link.link} target="blank" className="w-11 h-11 flex items-center justify-center hover:bg-skin-fill-secondary hover:bg-opacity-70 rounded-full transition ease-in-out delay-100">
            {iconsMap[link.name]}
        </a>
    );
}
